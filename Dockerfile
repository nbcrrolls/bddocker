...
RUN export DEBIAN_FRONTEND=noninteractive && \
    ...
    make all && \
    curl -sk $BD_URL/browndye2.tar.gz | tar xzf - -C /opt && \
    cd /opt/browndye2 && curl -sk -sO $BD_URL/browndye2/doc/fixes.html && \
    make all && \
    ...
    mkdir lib && cd lib && \
    curl -sk -sO ${BD_URL}/coffdrop.xml.gz && \
    curl -sk -sO ${BD_URL}/connectivity.xml && \
    curl -sk -sO ${BD_URL}/map.xml && \
    curl -sk -sO ${BD_URL}/charges.xml && \
...
ENV PDB2PQR_PATH /usr/local

# set up browndye user
ENV user=browndye
RUN groupadd ${user} && useradd -g ${user} -l -ms /bin/bash ${user}
#USER ${user}
#ENV PATH ${APBS_PATH}/bin:${PDB2PQR_PATH}/bin:${BD2_PATH}/bin:${PATH}
ENV PATH ${PDB2PQR_PATH}/bin:${BD2_PATH}/bin:${PATH}:${APBS_PATH}/bin
